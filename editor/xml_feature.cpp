...
// REPLACE the existing ToXML function with this:
XMLFeature ToXML(osm::EditableMapObject const & object, bool serializeType, editor::EditorConfig const & config)
{
  ...
  object.GetNameMultilang().ForEach([&toFeature](uint8_t lang, string_view name) { toFeature.SetName(lang, name); });
...
{
  ASSERT_EQUAL(XMLFeature::Type::Node, xml.GetType(), ("Only new nodes can be created."));
  object.SetPointType();
  object.SetMercator(xml.GetMercatorCenter());
  xml.ForEachName([&object](std::string_view lang, std::string_view name)
  { object.SetName(name, StringUtf8Multilang::GetLangIndex(lang)); });
